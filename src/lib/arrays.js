...
const reverseWordsInArray = strings => {
  ...
  for (let i = 0; i < strings.length; i++) {
    const splitWord = strings[i].split(''); // This splits the string into individual characters in an array.
    const reverseWord = splitWord.reverse(); // This reverses the array order e.g ["D", "e", "a", "n"] -> ["n", "a", "e", "D"]
    const joinedWord = reverseWord.join(''); // This joins the array into one string, with nothing separating each character.
...
const removeNthElement2 = (index, array) => {
  ...
  for (let i = 0; i < array.length; i++) {
    if (array.indexOf(array[i]) != index) {
      // If the index number of the ith element of array does not equal index, push it into newArray.
...
const elementsStartingWithAVowel = strings => {
  const vowelStrings = [];
  for (let i = 0; i < strings.length; i++) {
    const lowerCase = strings[i].toLowerCase();
    if (
      ...
      // If the first character of the lowerCase string is a/e/i/o/u, push to vowelStrings array.
...
const sortByLastLetter = strings => {
  function endOfWord(word1, word2) {
    if (word1.slice(-1) < word2.slice(-1)) {
      // If the last letter of word 1 is < word 2, put w1 before w2.
      return -1;
    }
    if (word1.slice(-1) > word2.slice(-1)) {
